...
*/
trigger CountryMappingContactTrigger on Contact (after insert, before update) {
/*
	CountryConfig__c cntryconfig = CountryConfig__c.getinstance('Settings'); 	
	if(cntryconfig.Contact_Event_Trigger_Enabled__c == True && !ddcLib1.RuntimeState.isInFutureContext){
		
		list<Country_Mapping__CountryFieldSet__c> fieldmapList = new list<Country_Mapping__CountryFieldSet__c>(
		[SELECT Country_Mapping__FieldName__c FROM Country_Mapping__CountryFieldSet__c WHERE Country_Mapping__ObjectName__c = 'Contact']);
		...
		List<Contact> afterInsertacts = new List<Contact>();
			...
			for(Contact act: Trigger.new){
				...
				for(String fieldName: actIdAndCountry.keyset()){
					if(trigger.isbefore && accIdAndCountryISO.get(fieldName.toUppercase()) != null){
						...
						Contact  ac = new Contact (Id=act.Id);
						//ac.ShippingCountry = accIdAndCountryISO.get(act.Id);
