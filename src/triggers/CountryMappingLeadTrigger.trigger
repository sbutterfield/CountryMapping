...
*/
trigger CountryMappingLeadTrigger on Lead (after insert, before update) {
/*
	CountryConfig__c cntryconfig = CountryConfig__c.getinstance('Settings'); 	
	if(cntryconfig.Lead_Event_Trigger_Enabled__c == True && !ddcLib1.RuntimeState.isInFutureContext){
		
		list<Country_Mapping__CountryFieldSet__c> fieldmapList = new list<Country_Mapping__CountryFieldSet__c>(
		[SELECT Country_Mapping__FieldName__c FROM Country_Mapping__CountryFieldSet__c WHERE Country_Mapping__ObjectName__c = 'Lead']);
		...
		List<Lead> afterInsertacts = new List<Lead>();
		
		for(Lead act: Trigger.new){
			try{
